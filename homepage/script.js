...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (searchForm) {
        ...
        console.warn("Elemento #searchForm non trovato nel DOM.");
    }
    ...
    const pagePaths = {
        "Homepage": "homepage/index.html", "Neon Genesis Evangelion (anime)": "homepage/anime.html",
    ...
    function loadRecentSearches() {
        ...
        console.log("Caricamento ricerche recenti:", savedSearches); // Debug
        if (savedSearches) {
    ...
    function saveRecentSearches() {
        console.log("Salvataggio ricerche recenti:", recentSearches.slice(0, 3)); // Debug
        localStorage.setItem("recentSearches", JSON.stringify(recentSearches.slice(0, 3)));
    }

    function updateRecentSearches() {
        console.log("Aggiornamento ricerche recenti:", recentSearches);
        const recentSearchesContainer = document.getElementById("suggestions-box");

        if (!recentSearchesContainer) {
            console.warn("Elemento #suggestions-box non trovato nel DOM.");
            return;
        }
        ...
        console.log("Contenuto HTML generato:", htmlContent);
        recentSearchesContainer.innerHTML = htmlContent;

        attachRecentSearchClickEvents();
        attachRecentSearchNavigationEvents(); // ✅ Aggiunto per far funzionare il click sulle ricerche recenti
    }
    window.updateRecentSearches = updateRecentSearches;

    function attachRecentSearchNavigationEvents() {
        document.querySelectorAll(".recent-item").forEach(item => {
            item.addEventListener("click", function () {
                const searchQuery = this.getAttribute("data-item"); // Ottiene il testo della ricerca
                if (searchQuery) {
                    handleSearch(searchQuery); // Esegue la ricerca
                }
            });
        });
    }
    ...
    function attachRecentSearchClickEvents() {
        console.log("Aggiunta eventi click alle ricerche recenti");

        // ❌ Prima di aggiungere nuovi eventi, rimuoviamo quelli vecchi
        document.querySelectorAll(".remove-recent").forEach(button => {
            button.replaceWith(button.cloneNode(true));
        });

        // ✅ Ora assegniamo i nuovi eventi senza duplicati
        document.querySelectorAll(".remove-recent").forEach(button => {
            button.addEventListener("click", function (event) {
                event.stopPropagation();
                const index = parseInt(this.getAttribute("data-index"), 10);

                console.log("Cliccato su X, indice:", index);
                console.log("Stato prima della rimozione:", [...recentSearches]);

                if (!isNaN(index) && index >= 0 && index < recentSearches.length) {
                    recentSearches.splice(index, 1);
                    console.log("Stato dopo la rimozione:", [...recentSearches]);
                    saveRecentSearches();
                    updateRecentSearches(); // ✅ Ora gli indici verranno aggiornati correttamente
                }
            });
        });
    }
    ...
    if (searchInput) {
        searchInput.addEventListener("focus", () => {
            ...
            // ✅ Mostra il box SOLO se ci sono ricerche recenti
    ...
    function updateSuggestionsBox(query) {
        let htmlContent = "";
        if (query.length === 0) {
            htmlContent = "<div style='color: #aaa;'>cerca nel sito...</div>";
        } else {
        ...
        attachSuggestionClickEvents(); // ✅ Aggiunto qui per collegare gli eventi click
    }

    function attachSuggestionClickEvents() {
        document.querySelectorAll(".suggestion-item, .suggestion-highlight").forEach(suggestion => {
            suggestion.addEventListener("click", function () {
                const selectedText = this.getAttribute("data-item"); // Ottiene il testo del suggerimento cliccato
                if (selectedText && pagePaths[selectedText]) {
                    window.location.href = "/" + pagePaths[selectedText]; // Reindirizza alla pagina corretta
                } else {
                    console.warn("Nessun percorso trovato per:", selectedText);
                }
            });
        });
    }
    ...
    loadRecentSearches();
    console.log("Ricerche recenti caricate:", recentSearches); // Debug
    updateRecentSearches(); // Assicurati che questa funzione venga chiamata all'avvio
